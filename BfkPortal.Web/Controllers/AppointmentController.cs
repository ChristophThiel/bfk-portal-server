...
namespace BfkPortal.Web.Controllers
{
    ...
    {
        internal IAppointmentService Service;

        public AppointmentController()
        {
            Service = new AppointmentService(this.ModelState);
        }

        [HttpPost("add")]
        public async Task<IActionResult> Add([FromHeader] string authorization, [FromBody] AppointmentViewModel body)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);

            /*var result = Service.Add(await body.ToModel());
            if (!result.IsValid)
                return BadRequest(ModelState);*/
